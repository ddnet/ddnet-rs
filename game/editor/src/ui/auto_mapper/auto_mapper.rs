...
use crate::{
    ...
    tools::{
        tile_layer::auto_mapper::{
            TileLayerAutoMapper, TileLayerAutoMapperCheckGroup, TileLayerAutoMapperOperator,
            TileLayerAutoMapperRule, TileLayerAutoMapperRuleType, TileLayerAutoMapperRun,
            TileLayerAutoMapperTile, TileLayerAutoMapperTileExpr, TileLayerAutoMapperTileType,
            TileLayerAutoMapperVisuals, TileOffsetNonZero,
        },
        ...
        for y in 0..(run_tile.grid_size * 2 + 1) {
            ui.horizontal(|ui| {
                for x in 0..(run_tile.grid_size * 2 + 1) {
                    ...
                    fn find_tile_mut(
                        check_tiles: &mut HashMap<TileOffsetNonZero, TileLayerAutoMapperCheckGroup>,
                        x: i32,
                        y: i32,
                        offset: usize,
                    ) -> Option<&mut TileLayerAutoMapperCheckGroup> {
...
pub fn render(pipe: &mut UiRenderPipe<UserData>, ui: &mut egui::Ui, ui_state: &mut UiState) {
    ...
    auto_mapper.update(pipe.user_data.notifications);

    let window_res = Window::new("Auto-mapper-rule-creator").show(ui.ctx(), |ui| {
        let min_tile_size = 40.0;
            ...
            .and_then(|i| auto_mapper.resources.get_mut(i).map(|res| (i, res)))
        {
            ui.horizontal(|ui| {
                ...
                if ui.button("\u{f055} Add new rule").clicked() {
                    resource.rules.insert(
                        ...
                        TileLayerAutoMapperRuleType::EditorRule(TileLayerAutoMapperRule::default()),
                    );
                }
            });
            ...
            egui::ComboBox::new("auto-mapper-a-rule-selector", "Select rule to edit")
                ...
                .show_ui(ui, |ui| {
                    ui.vertical(|ui| {
                        let rules: BTreeSet<_> = resource.rules.keys().collect();
                        for r in rules {
                            if ui.add(egui::Button::new(r)).clicked() {
